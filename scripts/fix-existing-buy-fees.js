...
/**
 * Fix Existing Buy Token Fees
 * Add fee data to existing buy transactions that don't have fees
 */

const { databaseHelpers } = require('../src/lib/database');
const { calculateFee, creditFeeToAdmin } = require('../src/lib/fees');

async function fixExistingBuyFees() {
  console.log('🔧 Fixing Existing Buy Token Fees\n');
  ...
  try {
    ...
    console.log('\n2️⃣ Applying fees to existing buy transactions...');
    ...
    console.log('\n🎉 Existing buy token fees fixed successfully!');
    console.log('\n📋 Summary:');
    console.log('✅ Fee data added to existing buy transactions');
    console.log('✅ Fees credited to admin wallet');
    console.log('✅ Admin fees page will now show buy transaction history');
    console.log('✅ Future buy transactions will automatically include fees');

  } catch (error) {
    console.error('\n❌ Error fixing buy token fees:', error);
...
if (require.main === module) {
  fixExistingBuyFees().catch(console.error);
}

module.exports = { fixExistingBuyFees };
