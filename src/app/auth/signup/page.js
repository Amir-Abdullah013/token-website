    ...
    try {
      ...
      if (data.success) {
        if (data.requiresOTP) {
          ...
          // Fallback: account created without OTP (shouldn't happen in normal flow)
          setSuccessData({
            name: formData.name.trim(),
            email: formData.email.trim()
          });
  ...
  const handleOTPVerification = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        } else if (data.errorCode === 'OTP_NOT_FOUND') {
          setErrors({ otp: 'No OTP found. Please request a new OTP.' });
        } else {
            ...
            <form className="space-y-6" onSubmit={handleOTPVerification}>
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-slate-400">
                  Check your email ({formData.email}) for the 6-digit verification code
                </p>
              </div>
